...
/**
 * String/JSON TCP Protocol Adaptor
 */
@Component
@TbTcpTransportComponent
@Slf4j
public class StringTcpAdaptor implements TcpTransportAdaptor {
    ...
    public PostTelemetryMsg convertToPostTelemetry(TcpDeviceSessionContext ctx, ByteBuf inbound) throws AdaptorException {
        ...
        log.info("[{}] StringTcpAdaptor converting telemetry payload: {}", ctx.getSessionId(), payload);
        try {
            PostTelemetryMsg result = JsonConverter.convertToTelemetryProto(JsonParser.parseString(payload));
            log.info("[{}] Successfully converted telemetry", ctx.getSessionId());
            return result;
        } catch (IllegalStateException | JsonSyntaxException e) {
            log.error("[{}] Failed to decode telemetry", ctx.getSessionId(), e);
    ...
    public PostAttributeMsg convertToPostAttributes(TcpDeviceSessionContext ctx, ByteBuf inbound) throws AdaptorException {
        String payload = inbound.toString(StandardCharsets.UTF_8);
        log.info("[{}] StringTcpAdaptor converting attributes payload: {}", ctx.getSessionId(), payload);
        try {
            PostAttributeMsg result = JsonConverter.convertToAttributesProto(JsonParser.parseString(payload));
            log.info("[{}] Successfully converted attributes", ctx.getSessionId());
            return result;
        } catch (IllegalStateException | JsonSyntaxException e) {
            log.error("[{}] Failed to decode attributes", ctx.getSessionId(), e);
