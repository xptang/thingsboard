 ...
 * 开泵关阀请求
 * 帧代号：0x81
 * 数据段：1字节（1=泵2, 2=泵1, 3=阀门1, 4=阀门2, 5=阀门3）
 */
@Data
@Builder
@Slf4j
public class EelinkOpenPumpRequest implements EelinkCommandRequest {
    ...
    /**
     * 从RPC参数创建请求
     * RPC示例: {method:"openPump", params:{device:2}} 或 {method:"openPump", params:{device:"PUMP1"}}
     */
    public static EelinkOpenPumpRequest fromRpc(int requestId, JsonObject params) {
        DeviceType device = DeviceType.PUMP1; // 默认泵1
        ...
        return EelinkOpenPumpRequest.builder()
                .requestId(requestId)
                .device(device)
                .build();
    }
    ...
    public ByteBuf encodeToFrame(ByteBufAllocator allocator, byte[] deviceAddress) {
        ...
        log.info("Encoding openPump request: requestId={}, device={}({})", 
