...
@Data
@Slf4j
public class EelinkOpenPumpResponse implements EelinkCommandResponse {
    
    public static final byte FRAME_CODE = (byte) 0x81;
    
    private int requestId;
    private boolean success;
    private EelinkOpenPumpRequest.DeviceType device;
     ...
     */
    public static EelinkOpenPumpResponse parse(byte[] data, byte functionCode2) {
        EelinkOpenPumpResponse response = new EelinkOpenPumpResponse();
        ...
        if (success && data.length >= 1) {
            ...
            try {
                response.setDevice(EelinkOpenPumpRequest.DeviceType.fromCode(deviceCode));
        ...
        log.debug("Parsed openPump response: success={}, device={}, errorCode=0x{}", 
                 success, 
    ...
    public JsonObject toRpcResponse() {
        ...
        response.addProperty("command", "openPump");
        ...
        if (success) {
            ...
            response.addProperty("message", "开泵关阀成功");
        } else {
